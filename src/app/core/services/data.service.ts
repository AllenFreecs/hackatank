...
export class DataService {
  ...
  addAutomation(input: Omit<Automation, 'id'>): Automation {
    ...
    const automation: Automation = { ...input, id: nextId, trigger: input.trigger ?? 'Scheduled report generation', action: input.action ?? (input.automationType === 'Email Creation' ? 'Create email content' : `Export ${fileType ?? 'pdf'} report`), recipient: input.recipient ?? 'user@company.com', fileType, aiQuery: input.aiQuery ?? '', status: input.status ?? 'Enabled' };
  ...
  isAutomationDue(automation: Automation): boolean {
    if (!automation.lastRunAt) {
      return true;
    }
    const intervalMs = this.frequencyMilliseconds(automation.frequency);
    return Date.now() - new Date(automation.lastRunAt).getTime() >= intervalMs;
  }

  markAutomationRun(id: number): void {
    const nextAutomations = this.automationsSubject.value.map((automation) => automation.id === id ? { ...automation, lastRunAt: new Date().toISOString() } : automation);
    this.automationsSubject.next(nextAutomations);
    this.persistAutomations(nextAutomations);
  }
  ...
  exportAutomationResponse(automationId: number, name: string, type: 'excel' | 'word' | 'pdf', content: string): string {
    const outputPath = this.exportAssistantResponse(name, type, content);
    const fileName = outputPath.split('/').pop() ?? outputPath;
    this.generatedArtifactsSubject.next([
      { id: Date.now(), automationId, name: fileName, outputPath, type, createdAt: new Date().toISOString() },
      ...this.generatedArtifactsSubject.value
    ].slice(0, 25));
    const nextAutomations = this.automationsSubject.value.map((automation) => automation.id === automationId ? { ...automation, outputPath } : automation);
    this.automationsSubject.next(nextAutomations);
    this.persistAutomations(nextAutomations);
    return outputPath;
  }
  ...
  private frequencyMilliseconds(frequency: string): number {
    ...
    if (normalized.includes('week')) { return amount * 7 * 24 * 60 * 60 * 1000; }
